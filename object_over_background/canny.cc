...
void on_slider (int amount,void *data){
  ...
  for(int i = 0; i>= 0; i = hierarchy[i][0]){
    ...
    cv::drawContours(contours_img, contours, i, color, CV_FILLED, 8, hierarchy);
  }
  cv::imshow("Contours", contours_img);

  cv::Mat rects = cv::Mat::zeros(img->img.rows, img->img.cols, CV_8UC3);
  for (auto &point_set : contours){
    cv::Mat rect = boundingRect(point_set);
    cv::addWeighted(rect, 1, rects, 0);
  }
