...
int main (int argc, char** argv){
  ...
  cv::createTrackbar("size", "Trackbars", &canny.size, 100, on_slider, &canny);
  on_slider(0,&canny);
  cv::waitKey(0);
  return 0;
}
...
void on_slider (int amount,void *data){
  ...
  cv::Mat kernell = cv::getStructuringElement(cv::MORPH_RECT,cv::Size(size,size));
  cv::Mat dilated;
  cv::dilate(canny, dilated,kernell);
